...
def train(config, workdir):
  ...
  #수정
  train_ds, eval_ds= datasets.create_dataloader(config)
  num_data_train = len(train_ds.dataset)
  num_data_eval = len(eval_ds.dataset)
  ...
  for epoch in range(1, config.training.epochs):
    for step, batch in enumerate(train_ds, start=1):
      ...
      if step % config.training.log_freq == 0:
        ...
        global_step = num_data_train * epoch + step
    ...
    if epoch % config.training.eval_freq == 0:
      for step_eval, batch_eval in enumerate(eval_ds, start=1):
        batch_eval = scaler(batch_eval.to(config.device))
        loss_eval = train_step_fn(state, batch_eval)
        global_step_val = global_step_val  + 1
        logging.info("epoch: %d, step: %d, eval_loss: %.5e" % (epoch, step, loss_eval.item()))
        writer.add_scalar("eval_loss", scalar_value=loss_eval, global_step=global_step_val)
...
def evaluate(config,
  ...
  # train_ds, eval_ds= datasets.create_dataloader(config,evaluation=True)
  # num_data_train = len(train_ds.dataset)
  # num_data_eval = len(eval_ds.dataset)
